...
func (c *Client) FindEntertainmentFact(ctx context.Context) (facts.Diversity, error) {
	type (
		data struct {
			ID                 string `json:"id"`
	...
	if err != nil {
		return facts.Diversity{}, eris.Wrap(err, "creating request for entertainment facts failed")
	}

	res, err := c.httpClient.Do(req)
	if err != nil {
		if err != nil {
			return facts.Diversity{}, eris.Wrap(err, "request for entertainment facts failed")
		}
	}
